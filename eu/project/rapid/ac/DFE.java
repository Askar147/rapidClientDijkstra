...
public class DFE {
    ...
    private ExecLocation execLocation = ExecLocation.REMOTE;
...
//            new StreamThread(runAcRm.getErrorStream()).start();
    ...
    public Object execute(Method m, Object[] pValues, Object o) {
        ...
        try {
            ...
            log.info("" +
                    "Waiting for the result of the task with id=" + id +
...
//                e.printStackTrace();
        ...
        private Object runTask(Task task, OutputStream os, ObjectInputStream ois, ObjectOutputStream oos) {
            ...
            if (execLocation.equals(ExecLocation.LOCAL)) {
                ...
                setExecLocation(ExecLocation.LOCAL);
//                result = executeLocally(task);
    ...
    public void destroy() {
        ...
        log.info("DESTROY: VMM IP: " + vm.getIp() + " , VMM PORT: " + config.getVmmPort());

        // ------
        try (Socket vmmSocket = new Socket(vm.getIp(), config.getVmmPort());
            ...
            log.info("Notifying VMM " + vm.getIp() + ":" + config.getVmmPort() + "...");
            vmmOs.writeByte(RapidMessages.AS_RM_NOTIFY_VMM);
            vmmOs.writeLong(config.getUserid());
            vmmOs.writeInt(4);
            vmmOs.flush();

            vmmOs.close();
            vmmSocket.close();
            log.info("Offload deregistered message sent to VMM");
//
////            //TODO: REMOVE TO AND CONNECT TO ALREADY CREATED ACRM
//            if (runAcRm != null) {
//                runAcRm.destroy(); // Terminate the AC_RM process
//                log.info("AC_RM process destroyed.");
//            }
        } catch (IOException e) {
            log.info("Could not notify VMM: " + e);
        }
        //
    }
