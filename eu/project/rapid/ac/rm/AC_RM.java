...
public class AC_RM {
        ...
        if (connectedWithSlam) {
            try (ObjectOutputStream oos = new ObjectOutputStream(slamSocket.getOutputStream());
                ...
                if (response == RapidMessages.OK) {
                    ...
                    vm.setId((int) myId);
                    vm.setPort(port);
                    vm.setClonePortBandwidthTest(port-1);
    ...
    private static class ShutdownThread extends Thread {
        ...
        ShutdownThread(ServerSocket serverSocket) {
        ...
        public void run() {
            try {
                ...
                try (Socket vmmSocket = new Socket(config.getVmmIp(), config.getVmmPort());
                    ...
                    log.info("Notifying VMM " + config.getVmmIp() + ":" + config.getVmmPort() + "...");
...
//            config.setAcRmPort(serverSocket.getLocalPort());
            log.info("Started server on port " + serverSocket.getLocalPort());
            Runtime.getRuntime().addShutdownHook(new ShutdownThread(serverSocket));
